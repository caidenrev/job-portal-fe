...
interface Conversation {
    ...
    hr: { id: number, name: string, profileImageUrl?: string }
    messages: Message[]
}
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
                                        {conv.hr.profileImageUrl ? (
                                            <img src={conv.hr.profileImageUrl} className="w-full h-full rounded-full object-cover" alt="HR" />
                                        ) : (
                                            <User className="w-5 h-5 text-primary" />
                                        )}
                                    </div>
                                    <div className="overflow-hidden">
                                        <h4 className="font-semibold truncate text-sm">{conv.hr.name} <span className="text-xs text-muted-foreground font-normal">({conv.job.company.name})</span></h4>
            ...
            <Card className="md:col-span-2 flex flex-col h-full bg-background/50 backdrop-blur-sm">
                {activeConversation ? (
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground flex items-center gap-1.5">
                                        <Briefcase className="w-3.5 h-3.5" /> Pelamar posisi: {activeConversation.job.title} di {activeConversation.job.company.name}
                                    </p>
                                </div>
                            </div>
                        </CardHeader>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-muted-foreground opacity-60">
                        ...
                        <p>Pilih percakapan untuk mulai bertukar pesan</p>
                    </div>
                )}
            </Card>
